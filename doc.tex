...
Contrairement au cas sans variables cachées (Section~\ref{sec:sans_cachees}), cette fonction n'est pas quadratique en $\theta$ ; il n'existe donc pas de formule explicite pour l'estimateur du maximum de vraisemblance. Nous présentons deux stratégies d'optimisation : une descente de gradient directe sur $\ell_n$, puis l'algorithme Espérance-Maximisation (EM), qui exploite la structure probabiliste du modèle.
...
Sous des hypothèses de domination permettant de dériver sous le signe intégral, on calcule le gradient de $\ell_n$. Pour une observation $Y_i$, posons
\[
p(Y_i;\theta) = \int_{\mathcal{Z}} \frac{1}{(2\pi\sigma^2)^{p/2}} \exp\!\left( -\frac{\|Y_i-A_z\theta\|^2}{2\sigma^2} \right) f_Z(z)\,d\mu(z).
\]
Alors $\nabla_\theta \log p(Y_i;\theta) = \frac{\nabla_\theta p(Y_i;\theta)}{p(Y_i;\theta)}$. En dérivant sous l'intégrale,
\[
\nabla_\theta p(Y_i;\theta) = \int_{\mathcal{Z}} \frac{1}{(2\pi\sigma^2)^{p/2}} \exp\!\left( -\frac{\|Y_i-A_z\theta\|^2}{2\sigma^2} \right) \frac{A_z^\top (Y_i-A_z\theta)}{\sigma^2} f_Z(z)\,d\mu(z).
\]
Ainsi,
\[
\nabla_\theta \log p(Y_i;\theta) = \frac{1}{\sigma^2} \int_{\mathcal{Z}} A_z^\top (Y_i-A_z\theta) \, f_\theta(z\mid Y_i)\,d\mu(z),
\]
où la densité \textit{a posteriori} de $Z_i$ sachant $Y_i$ est donnée par la formule de Bayes :
\[
f_\theta(z\mid Y_i) = \frac{ f_Z(z) \exp\!\left( -\frac{\|Y_i-A_z\theta\|^2}{2\sigma^2} \right) }{ \int_{\mathcal{Z}} f_Z(z') \exp\!\left( -\frac{\|Y_i-A_{z'}\theta\|^2}{2\sigma^2} \right) d\mu(z') }.
\]
Finalement, le gradient de $\ell_n$ s'écrit
\[
\boxed{ \nabla \ell_n(\theta) = \frac{1}{\sigma^2} \sum_{i=1}^n \int_{\mathcal{Z}} A_z^\top (Y_i-A_z\theta) \, f_\theta(z\mid Y_i)\,d\mu(z) }.
\]

\subsubsection{Application au Cas 1 (translations 1D) et discrétisation}

Dans le Cas~1, $\mathcal{Z}=[0,2\pi)$, $f_Z$ est uniforme, et $(A_z\theta)(x)=\theta(x-z)$.  
Pour une mise en œuvre numérique, on discrétise l'espace latent en $M$ points $z_1,\dots,z_M$ (par exemple une grille régulière). L'intégrale est alors remplacée par une somme, et l'itération de gradient (à pas $\eta_k$) devient
\[
\theta^{(k+1)} = \theta^{(k)} + \frac{\eta_k}{\sigma^2} \sum_{i=1}^n \sum_{m=1}^M w_{i,m}^{(k)} A_{z_m}^\top \bigl( Y_i - A_{z_m}\theta^{(k)} \bigr),
\]
avec les poids normalisés
\[
w_{i,m}^{(k)} = \frac{ f_Z(z_m) \exp\!\left( -\frac{\|Y_i - A_{z_m}\theta^{(k)}\|^2}{2\sigma^2} \right) }{ \sum_{\ell=1}^M f_Z(z_\ell) \exp\!\left( -\frac{\|Y_i - A_{z_\ell}\theta^{(k)}\|^2}{2\sigma^2} \right) }.
\]

\paragraph{Convergence et limites.}
Sous des hypothèses classiques (gradient lipschitzien, pas $\eta_k$ choisi selon les règles usuelles), la suite $(\theta^{(k)})$ converge vers un point critique de $\ell_n$. Cependant, $\ell_n$ est généralement non convexe ; la convergence n'est donc pas garantie vers un maximum global. De plus, le choix du pas est délicat et le calcul du gradient nécessite une intégration (ou sommation) sur $\mathcal{Z}$. Ces limitations motivent l'algorithme EM.
...
L'algorithme EM exploite les données complètes $(Y_i,Z_i)$. La log‑vraisemblance complète est
\[
\log p(Y,Z;\theta)
= -\frac{np}{2}\log(2\pi\sigma^2)
- \frac{1}{2\sigma^2} \sum_{i=1}^n \|Y_i - A_{Z_i}\theta\|^2
+ \sum_{i=1}^n \log f_Z(Z_i).
\]
Si les $Z_i$ étaient observés, le problème serait quadratique en $\theta$ (moindres carrés). L'EM remplace les $Z_i$ par leur espérance conditionnelle.

\subsubsection{Discrétisation de l'espace latent}

Dans la pratique, on discrétise $\mathcal{Z}$ en un ensemble fini $\mathcal{Z}_{\text{disc}} = \{z_1,\dots,z_M\}$ (par exemple une grille régulière). Les intégrales deviennent des sommes. On suppose que $f_Z$ est une densité par rapport à la mesure $\mu$, et que $\mu$ est la mesure de comptage sur $\mathcal{Z}_{\text{disc}}$ après discrétisation.

\subsubsection{Définition de la fonction auxiliaire}

Soit $\theta^{(k)}$ l'estimation courante. On définit
\[
Q(\theta \mid \theta^{(k)}) = \E_{\theta^{(k)}}\!\bigl[ \log p(Y,Z;\theta) \mid Y \bigr],
\]
où l'espérance est prise sous la loi $p(Z \mid Y;\theta^{(k)})$, avec $Z$ à valeurs dans $\mathcal{Z}_{\text{disc}}$.

\paragraph{Étape E (Espérance).}
On calcule les poids a posteriori pour chaque $z \in \mathcal{Z}_{\text{disc}}$ :
\[
\tau_{i,z}^{(k)} = f_{\theta^{(k)}}(z \mid Y_i)
= \frac{ f_Z(z) \exp\!\left( -\frac{\|Y_i - A_z\theta^{(k)}\|^2}{2\sigma^2} \right) }{ \sum_{z'\in\mathcal{Z}_{\text{disc}}} f_Z(z') \exp\!\left( -\frac{\|Y_i - A_{z'}\theta^{(k)}\|^2}{2\sigma^2} \right) }.
\]
Ces poids vérifient $\sum_{z\in\mathcal{Z}_{\text{disc}}} \tau_{i,z}^{(k)} = 1$ pour tout $i$.

\paragraph{Étape M (Maximisation).}
À une constante additive près indépendante de $\theta$,
\[
Q(\theta \mid \theta^{(k)}) = -\frac{1}{2\sigma^2} \sum_{i=1}^n \sum_{z\in\mathcal{Z}_{\text{disc}}} \tau_{i,z}^{(k)} \|Y_i - A_z\theta\|^2.
\]
Cette fonction est quadratique convexe en $\theta$. Calculons son gradient :
\[
\nabla_\theta Q(\theta \mid \theta^{(k)}) = -\frac{1}{\sigma^2} \sum_{i=1}^n \sum_{z} \tau_{i,z}^{(k)} A_z^\top (Y_i - A_z\theta).
\]
En annulant le gradient, on obtient les équations normales :
\[
\left( \sum_{i=1}^n \sum_{z\in\mathcal{Z}_{\text{disc}}} \tau_{i,z}^{(k)} A_z^\top A_z \right) \theta = \sum_{i=1}^n \sum_{z\in\mathcal{Z}_{\text{disc}}} \tau_{i,z}^{(k)} A_z^\top Y_i.
\]
La matrice $M^{(k)} = \sum_{i,z} \tau_{i,z}^{(k)} A_z^\top A_z$ est symétrique semi-définie positive. On suppose qu'elle est inversible (ce qui est vérifié en pratique si les opérateurs $A_z$ couvrent suffisamment l'espace $E$ et si $n$ est assez grand). La mise à jour EM est alors
\[
\boxed{ \theta^{(k+1)} = \bigl(M^{(k)}\bigr)^{-1} \left( \sum_{i=1}^n \sum_{z\in\mathcal{Z}_{\text{disc}}} \tau_{i,z}^{(k)} A_z^\top Y_i \right) }.
\]

\subsubsection{Propriétés théoriques}

\begin{proposition}[Dempster, Laird \& Rubin, 1977]
L'algorithme EM augmente la log‑vraisemblance marginale à chaque itération :
\[
\ell_n(\theta^{(k+1)}) \ge \ell_n(\theta^{(k)}).
\]
\end{proposition}

\begin{proof}
Soit $q$ une densité de probabilité arbitraire sur $\mathcal{Z}_{\text{disc}}$ (par rapport à la mesure de comptage), c'est‑à‑dire $\sum_{z} q(z)=1$, $q(z)\ge 0$. En multipliant l'intégrande par $q(z)/q(z)$ et en appliquant l'inégalité de Jensen (concavité de $\log$), on a
\[
\log p(Y;\theta) = \log \left( \sum_{z\in\mathcal{Z}_{\text{disc}}} \frac{p(Y,Z;\theta)}{q(z)} \, q(z) \right)
\ge \sum_{z} q(z) \log\frac{p(Y,Z;\theta)}{q(z)}.
\]
Choisissons $q(z) = p(z\mid Y;\theta^{(k)})$. L'inégalité devient
\[
\log p(Y;\theta) \ge \sum_{z} p(z\mid Y;\theta^{(k)}) \log p(Y,Z;\theta)
- \sum_{z} p(z\mid Y;\theta^{(k)}) \log p(z\mid Y;\theta^{(k)}).
\]
...
Toute valeur d'adhérence de la suite $(\theta^{(k)})_{k\ge0}$ est un point critique de $\ell_n$ :
\[
\nabla\ell_n(\theta)=0.
\]
\end{theorem}

Ainsi, l'algorithme EM converge (sous des conditions de régularité) vers un point stationnaire, mais la non‑convexité du problème n'assure pas l'obtention d'un maximum global.

% ----------------------------------------------------------------------------
\subsection{Comparaison des deux approches}
\label{subsec:comparaison}

La descente de gradient agit directement sur $\ell_n$ ; elle est simple à implémenter mais sensible au choix du pas et à la non‑convexité. Le coût d'une itération pour le Cas 1 discrétisé est $O(nMp)$ (amorti par FFT).

L'algorithme EM exploite la structure probabiliste sous‑jacente. Chaque itération résout un problème quadratique (moindres carrés pondérés). La construction de la matrice $M^{(k)}$ coûte $O(nMdp)$ (produits $A_z\theta$ et $A_z^\top Y_i$), et sa résolution par inversion directe coûte $O(d^3)$. Bien que plus coûteux par itération, l'EM requiert souvent moins d'itérations et offre une meilleure stabilité numérique.

Dans le cadre du Cas 1, des expériences numériques (non détaillées ici) montrent que l'EM est plus robuste au bruit et produit des reconstructions de meilleure qualité que la descente de gradient simple.
%
...
\[
M^{(k)} = \sum_{i=1}^n \sum_{z\in\mathcal{Z}_{\text{disc}}} \tau_{i,z}^{(k)} A_z^\top A_z,
\]
apparaissant dans l'étape M de l'algorithme EM. Ici, $\mathcal{Z}_{\text{disc}} = \{z_1,\dots,z_M\}$ est une discrétisation finie de l'espace latent (cf. Section~\ref{sec:optimisation}).
...
\[
\ell_n^{\mathrm{reg}}(\theta) = \ell_n(\theta) - \lambda R(\theta),
\]
où :
\begin{itemize}
...
\[
\theta^{(k+1)} = \arg\max_{\theta\in E} \bigl( Q(\theta\mid\theta^{(k)}) - \lambda R(\theta) \bigr).
\]
...
\[
\min_{\theta\in E} \frac{1}{2\sigma^2} \sum_{i=1}^n \sum_{z\in\mathcal{Z}_{\text{disc}}} \tau_{i,z}^{(k)} \|Y_i - A_z\theta\|^2 + \frac{\lambda}{2}\|\theta\|^2.
\]

En annulant le gradient, on obtient les équations normales régularisées :
\[
\left( \sum_{i=1}^n \sum_{z\in\mathcal{Z}_{\text{disc}}} \tau_{i,z}^{(k)} A_z^\top A_z + \lambda\sigma^2 I \right) \theta = \sum_{i=1}^n \sum_{z\in\mathcal{Z}_{\text{disc}}} \tau_{i,z}^{(k)} A_z^\top Y_i.
\]

La mise à jour EM devient donc
\[
\theta^{(k+1)} = \left( \sum_{i=1}^n \sum_{z\in\mathcal{Z}_{\text{disc}}} \tau_{i,z}^{(k)} A_z^\top A_z + \lambda\sigma^2 I \right)^{-1} \left( \sum_{i=1}^n \sum_{z\in\mathcal{Z}_{\text{disc}}} \tau_{i,z}^{(k)} A_z^\top Y_i \right).
\]
...
Soient $\mu_1 \ge \mu_2 \ge \dots \ge \mu_d > 0$ les valeurs propres de la matrice
\[
M^{(k)} = \sum_{i=1}^n \sum_{z\in\mathcal{Z}_{\text{disc}}} \tau_{i,z}^{(k)} A_z^\top A_z.
\]
...
La régularisation remplace les valeurs propres $\mu_j$ par $\mu_j + \lambda\sigma^2$, ce qui évite les divisions par de très petites quantités et stabilise l'inversion.
