...
Sous des hypothèses de domination permettant la dérivation sous le signe intégral, on calcule le gradient de $\ell_n$. Pour une observation $Y_i$, posons
\[
p(Y_i;\theta) = \int_{\mathcal{Z}} \frac{1}{(2\pi\sigma^2)^{p/2}} \exp\!\left( -\frac{\|Y_i-A_z\theta\|^2}{2\sigma^2} \right) f_Z(z)\,d\mu(z).
\]
Alors
\[
\nabla_\theta \log p(Y_i;\theta) = \frac{\nabla_\theta p(Y_i;\theta)}{p(Y_i;\theta)}.
\]
En dérivant sous l’intégrale,
\[
